...
#pragma unroll
  ...
  //      end_dims_no_mu[2]); tune_and_launch_for<3>("openBC", start, end_dims,
  //        KOKKOS_LAMBDA(const index_t i0, const index_t i1, const index_t i2)
  //        {
  //          if constexpr (mu == 0) IndexArray<4> site{dimensions[0], i0, i1,
  //          i2}; if constexpr (mu == 1) IndexArray<4> site{i0, dimensions[1],
  //          i1, i2}; if constexpr (mu == 2) IndexArray<4> site{i0, i1,
  //          dimensions[2], i2}; if constexpr (mu == 3) IndexArray<4> site{i0,
  //          i1, i2, dimensions[3]}; field(site, mu) =
