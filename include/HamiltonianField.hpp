...
namespace klft {
  ...
  public:
    ...
    // KOKKOS_INLINE_FUNCTION void operator()(update_momentum_s, const int &x, const int &y, const int &z, const int &t, const int &mu) const {
    ...
    void update_momentum(AdjointField<T,Adjoint,Ndim,Nc> deriv, const T dtau) {
      auto BulkPolicy = Kokkos::MDRangePolicy<Kokkos::Rank<5>>({0,0,0,0,0},{adjoint_field.get_dim(0),adjoint_field.get_dim(1),adjoint_field.get_dim(2),adjoint_field.get_dim(3),adjoint_field.get_Ndim()});
    ...
    // KOKKOS_INLINE_FUNCTION void operator()(update_gauge_s, const int &x, const int &y, const int &z, const int &t, const int &mu) const {
    ...
    T kinetic_energy() {
      ...
      auto BulkPolicy = Kokkos::MDRangePolicy<Kokkos::Rank<5>>({0,0,0,0,0},{adjoint_field.get_dim(0),adjoint_field.get_dim(1),adjoint_field.get_dim(2),adjoint_field.get_dim(3),adjoint_field.get_Ndim()});
      Kokkos::parallel_reduce("kinetic_energy", BulkPolicy, KOKKOS_LAMBDA(const int x, const int y, const int z, const int t, const int mu, T &local_sum) {
        Adjoint tmp = adjoint_field.get_adjoint(x,y,z,t,mu);
