...
namespace klft
{
  ...
  template <size_t N, typename Tin>
  KOKKOS_FORCEINLINE_FUNCTION
  Kokkos::Array<Tin, N> operator*(const Kokkos::Array<Tin, N> &a, const Tin &b) {
	Kokkos::Array<Tin, N> c;
    #pragma unroll
  ...
  template <size_t N, typename Tin>
  KOKKOS_FORCEINLINE_FUNCTION
  Kokkos::Array<Tin, N> operator*(const Tin &b, const Kokkos::Array<Tin, N> &a ) {
    return a*b;
}

  template <size_t Nc, typename Tin>
  KOKKOS_FORCEINLINE_FUNCTION
  void operator*=(SUNAdj<Nc> &a, const Tin &b) {
    #pragma unroll
  ...
  template <size_t Nc>
  KOKKOS_FORCEINLINE_FUNCTION
  SUNAdj<Nc> operator+(const SUNAdj<Nc> &a, const SUNAdj<Nc> &b) {
    SUNAdj<Nc> c;
    #pragma unroll
    for (size_t i = 0; i < getNcAdj(Nc); ++i) {
      c[i] = a[i] + b[i];
  ...
  template <size_t Nc>
  KOKKOS_FORCEINLINE_FUNCTION
  void operator+=(SUNAdj<Nc> &a, const SUNAdj<Nc> &b) {
    #pragma unroll
    for (size_t i = 0; i < getNcAdj(Nc); ++i) {
      a[i] += b[i];
    }
  ...
  template <size_t Nc>
  KOKKOS_FORCEINLINE_FUNCTION
  real_t norm2(const SUNAdj<Nc> &a) {
    real_t c = 0.0;
    #pragma unroll
    for (size_t i = 0; i < getNcAdj(Nc); ++i) {
      c += a[i] * a[i];
  ...
  SUNAdj<3> traceT(const SUN<3> &a) {
    ...
    c[7] = 0.5 * ((-a[0][0].imag() - a[1][1].imag() + 2.0 * a[2][2].imag()) * SQRT3INV);
    return c;
  }
  ...
  // constexpr
  // KOKKOS_FORCEINLINE_FUNCTION
  // SUN<3> get_SU3_from_adj(const SUNAdj<3> &a) {
  //   SUN<3> c;
  ...
  // constexpr
  // KOKKOS_FORCEINLINE_FUNCTION
  // real_t imag_det_SU3(const SUNAdj<3> &a) {
  //   real_t d = - 2.0*SQRT3INV * a[7] * 
  //              (a[7] * a[7] / 3.0 - a[2] * a[2])
  //              - 2.0 * (a[1] * a[3] * a[6]
  //              - a[0] * a[3] * a[5] - a[1] * a[4] * a[5]
  //              - a[0] * a[4] * a[6]);
