...
struct HMC_Params {
  ...
  HMC_Params(size_t _n_steps, real_t _tau) : n_steps(_n_steps), tau(_tau) {}
};

template <class RNG, int Ndim = 4, int Nc = 3> class HMC {
public:
  ...
  HMC(const HMC_Params _params, RNG _rng,
      std::uniform_real_distribution<T> _dist, std::mt19937 _mt)
      : params(_params), rng(_rng), dist(_dist), mt(_mt) {}

  void add_gauge_monomial(const T _beta, const unsigned int _time_scale) {
    monomials.emplace_back(
        std::make_unique<GaugeMonomial<T, Group, Adjoint, Ndim, Nc>>(
            _beta, _time_scale));
  }

  void add_kinetic_monomial(const unsigned int _time_scale) {
    monomials.emplace_back(
        std::make_unique<KineticMonomial<T, Group, Adjoint, Ndim, Nc>>(
            _time_scale));
  }

  void add_hamiltonian_field(
      const HamiltonianField<T, Group, Adjoint, Ndim, Nc> _hamiltonian_field) {
  ...
  bool hmc_step() {
    ...
    GaugeField<T, Group, Ndim, Nc> gauge_old(
        hamiltonian_field.gauge_field.dims);
