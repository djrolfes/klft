...
int JTBC_execute(const std::string &input_file,
  ...
  bool inputFileParsedCorrectly =
      (parseInputFile(input_file, output_directory, gaugeObsParams) &&
       ...
       parseInputFile(input_file, output_directory, jtbcSimLogParams));
  ...
  jtbcParams.gauge_params = gaugeMonomialParams;
  ...
  if (hmcParams.coldStart) {
    if (hmcParams.Ndims == 4) {
      ...
      dParams.defect_value = 1.0;
      if (hmcParams.Nc == 1) {
        using DGaugeFieldType =
            DeviceGaugeFieldType<4, 1, GaugeFieldKind::JTBC>;
        ...
        using JTBC = JTBC<DGaugeFieldType, DAdjFieldType, RNGType>;
        ...
        using DGaugeFieldType =
            DeviceGaugeFieldType<4, 2, GaugeFieldKind::JTBC>;
      ...
      if (hmcParams.Nc == 1) {
        using DGaugeFieldType =
            DeviceGaugeFieldType<3, 1, GaugeFieldKind::JTBC>;
        ...
        using DGaugeFieldType =
            DeviceGaugeFieldType<3, 2, GaugeFieldKind::JTBC>;
      ...
      if (hmcParams.Nc == 1) {
        using DGaugeFieldType =
            DeviceGaugeFieldType<2, 1, GaugeFieldKind::JTBC>;
        ...
        using DGaugeFieldType =
            DeviceGaugeFieldType<2, 2, GaugeFieldKind::JTBC>;
