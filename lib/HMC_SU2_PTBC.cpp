...
namespace klft {
  ...
  void HMC_SU2_4D_PTBC(const size_t &LX, const size_t &LY, const size_t &LZ, const size_t &LT,
    ...
    // Write header line
    outfile << "traj, accept, plaquette, time, acceptance rate, topological charge, [hmc acceptances], swap start, swap_acceptance, [swap acceptances], [swap_track], [cr], [delta S swap]" << std::endl;
    ...
    Kokkos::initialize();
    {
      ...
      for(size_t i = 0; i < n_traj; i++) {
        ...
        if(outfilename != "") {
          ...
          std::cout << "traj, accept, plaquette, time, acceptance rate, topological charge, [hmc acceptances], swap start, swap_acceptance, [swap acceptances], [swap_track], [cr], [delta S swap]" << std::endl;
