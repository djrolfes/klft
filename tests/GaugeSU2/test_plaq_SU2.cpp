...
int run_benchmark(const size_t stream_array_size) {
  ...
  printf("Plaquette FLOPS:           %11.4f Gflop/s\n",
         ...
         ( (2 * 3 * (8 + 4)) +  // two su2 multiplications
           (2 * (8 + 4))     +  // one su2 multiplcation (diagonal elements only)
           (2) )                 // trace accumulation (our plaquette is complex but we're interested only in the real part)
         / plaquetteTime);
...
int main(int argc, char *argv[]) {
  printf(HLINE);
  printf("SU(2) GaugeField 4D plaquette kernel test and benchmark\n");
  printf(HLINE);
