...
std::string printSUNAdj(const SUNAdj<Nc> &e) {
	std::ostringstream out;
	out << "[";
	for (int i = 0; i < getNcAdj(Nc); ++i){
		out << e[i];
	if (i < getNcAdj(Nc)-1){out << ", ";}
	}
	out << "]";
	return out.str();
}
...
SUN<Nc> ahermU = make_antiherm(U);
  std::cout << "Anti herm SUN: " << printSUN<Nc>(ahermU) << "\n";
SUNAdj<Nc> ahermUAdj = traceT(ahermU);
std::cout << "Anti herm Adjoint: " << printSUNAdj<Nc>(ahermUAdj) << "\n";
